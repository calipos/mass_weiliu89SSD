...
bool ReadTwoImageToSegDatum(const string& filename,
    const string& labelfile, const int height, const int width,
    const int min_dim, const int max_dim, const bool is_color,
    SegmentDatum* seg_datum) {  
  cv::Mat cv_img = ReadImageToCVMat(filename, height, width, min_dim, max_dim, true);
  if (cv_img.data) {
      if ( cv_img.channels() == 3 && !height && !width && !min_dim && !max_dim ){
          EncodeCVMatToDatum(cv_img, "jpg", seg_datum->mutable_imgdata());
          seg_datum->mutable_imgdata()->set_label(-1);
        }
  }else {
    LOG(WARNING)<<"SegmentDatum not standard.";
    return false;
  }

cv::Mat cv_mask = ReadImageToCVMat(labelfile, height, width, min_dim, max_dim, false);
  //LOG(INFO)<<"cv_mask().channels() = "<<cv_mask.channels();
  //LOG(INFO)<<"cv_mask().height() = "<<cv_mask.rows;
  //LOG(INFO)<<"cv_mask().width() = "<<cv_mask.cols;
 
  if (cv_mask.data) {
      if ( cv_mask.channels() == 1 && !height && !width && !min_dim && !max_dim ){
          EncodeCVMatToDatum(cv_mask, "bmp", seg_datum->mutable_maskdata());
          seg_datum->mutable_maskdata()->set_label(-1);
        }
  }else {
    LOG(WARNING)<<"SegmentDatum not standard.";
    return false;
  }
  //LOG(INFO)<<"imgdata().channels() = "<<seg_datum->imgdata().channels();
  //LOG(INFO)<<"imgdata().height() = "<<seg_datum->imgdata().height();
  //LOG(INFO)<<"imgdata().width() = "<<seg_datum->imgdata().width();
  //LOG(INFO)<<"imgdata().label() = "<<seg_datum->imgdata().label();
  //LOG(INFO)<<"imgdata().encoded() = "<<seg_datum->imgdata().encoded();
  //LOG(INFO)<<"maskdata().channels() = "<<seg_datum->maskdata().channels();
  //LOG(INFO)<<"maskdata().height() = "<<seg_datum->maskdata().height();
  //LOG(INFO)<<"maskdata().width() = "<<seg_datum->maskdata().width();
  //LOG(INFO)<<"maskdata().label() = "<<seg_datum->maskdata().label();
  //LOG(INFO)<<"maskdata().encoded() = "<<seg_datum->maskdata().encoded();
