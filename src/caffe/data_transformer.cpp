...
template<typename Dtype>
void DataTransformer<Dtype>::Transform(const Datum& img_datum,
                                       Blob<Dtype>* img_transformed_blob,
                                       const Datum& mask_datum,
                                       Blob<Dtype>* mask_transformed_blob) {
  bool do_mirror;
  Transform(img_datum, img_transformed_blob, mask_datum, mask_transformed_blob, &do_mirror);
}
...
void DataTransformer<Dtype>::Transform(const Datum& img_datum,
      ...
      cv_img = DecodeDatumToCVMatNative(img_datum);
      cv_seg = DecodeDatumToCVMatNative(mask_datum);
      int crop_h_off=0;
      int crop_w_off=0;
 
      TransformForSegmentImage(cv_img, transformed_img_blob, do_mirror,&crop_h_off,&crop_w_off);
      TransformForSegmentMask(cv_seg, transformed_mask_blob, *do_mirror,crop_h_off,crop_w_off);
      return;
#ifdef USE_OPENCV
#else
#endif  // USE_OPENCV
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformForSegmentImage(const cv::Mat& cv_img,
                                       Blob<Dtype>* transformed_blob,
                                       bool* do_mirror,
                                       int *crop_h_off,
                                       int *crop_w_off) {
...
//the do_mirror is same as the image
template<typename Dtype>
void DataTransformer<Dtype>::TransformForSegmentMask(const cv::Mat& cv_img,
                                       Blob<Dtype>* transformed_blob,
                                       bool do_mirror,
                                       int crop_h_off,
                                       int crop_w_off) {
  ...
  if (param_.has_noise_param()) {//the segment mask is not to be noised! so the cv_noised_image just = cv_resized_image
    //cv_noised_image = ApplyNoise(cv_resized_image, param_.noise_param());
    cv_noised_image = cv_resized_image;
  } else {
    cv_noised_image = cv_resized_image;
